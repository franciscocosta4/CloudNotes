    ...
    <x-app-layout>
        ...
        <main class="main">
            ...
            <div id="share-container" class="main-content" style="display: {{ request('query') ? 'none' : 'block' }}">
                ...
                <a href="{{ route('notes.create')}}" id="share-button"
                    style="margin:auto;background-color:#0F044C; color:white; border: none; padding: 8px 21px; cursor: pointer; border-radius: 8px; display: flex; align-items: center; justify-content: center; text-align: center; transition: background-color 0.3s, transform 0.3s;">
                    Partilhar
                </a>
            </div>
                                        ...
                                        <a id="link-note" href="{{ url('note/' . $result->slug) }}">
                                            <div id="note-summary-{{ $result->id }}" class="note-summary" style="width: 600px;">
                                                ...
                                                <p><strong>Utilizador:</strong>
                                                    {{ $result->user?->name ?? 'Utilizador não encontrado' }}</p>
            ...
            <button id="back-button" class="btn btn-secondary"
                style="display: {{ request('query') ? 'inline-block' : 'none' }};"
                onclick="window.location.href='{{ url('/dashboard') }}'">Voltar</button>
        </main>
        </div>
    </x-app-layout>
</body>
